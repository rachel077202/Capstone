...
### Set Up
* Download drone_control.py from this repo.
* In terminal go to the directory with the PX4-Autopilot folder then run the following commands.
```bash
cd PX4-Autopilot
PX4_SYS_AUTOSTART=10040 make px4_sitl_default none
```
* Open QGroundControl
* In a new terminal window run the following command in the directory containing the drone_control.py program.
```bash
